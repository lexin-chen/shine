...
import sys
sys.path.insert(0, '../../../../')
from src.tools.bts import rep_sample
            ...
            if self.sampling:
                if self.sampling == 'diversity':
                    if self.frame_cutoff <= len(traj):
                        ...
                        self.pathways[traj_idx] = traj # shouldn't it throw an error?
                elif self.sampling == 'quota':
                    if self.frame_cutoff <= len(traj):
                        n_frames = int(self.frac * len(traj))
                        rep_idx = rep_sample(traj, n_frames, n_frames)
