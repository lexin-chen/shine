...
for frac in [1]:
    ...
    for metric in metrics:
        ...
        for li in links:
            ...
            tick_positions = np.linspace(80, 550, num_ticks, dtype=int)
            tick_labels = [custom_label for custom_label in custom_labels]
            ax = plt.gca()
            
            for i, label in enumerate(custom_labels):
                ax.text(tick_positions[i], -0.03, label, color='black', ha='center', va='top', transform=ax.get_xaxis_transform(), fontsize=10, fontweight='bold')
