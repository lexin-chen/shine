...
def rep_sample(data, n_objects = None, metric = 'MSD', nbins = 10, nsample = 100, hard_cap = True):
...
for frac in [0.25, 0.5, 1]:
    ...
    for file in glob.glob("data/*.npy"):
        ...
        if rep_sampling:
            if len(frames) >= 50:
                ...
                s = rep_sample(frames, nbins = nf, nsample = nf)
    ...
    for metric in metrics:
        ...
        for li in links:
            ...
            tick_positions = np.linspace(800, 2800, num_ticks, dtype=int)
            tick_labels = [custom_label for custom_label in custom_labels]
            ax = plt.gca()
            
            for i, label in enumerate(custom_labels):
                ax.text(tick_positions[i], -0.03, label, color=colors[i+1], ha='center', va='top', transform=ax.get_xaxis_transform(), fontsize=10, fontweight='bold')
            
            # New code
            dendrogram(link, no_labels=True)
            plt.savefig('rep_dendro_{}_{}.png'.format(metric, int(frac*100)), bbox_inches='tight', dpi=500, pad_inches=0.1)
            plt.close()
            ...
            ##    st += '{:4}'.format(l)
            #
            #with open('paths_div_scan_{}_{}.txt'.format(metric, li), 'a') as outfile:
            #    outfile.write(st)